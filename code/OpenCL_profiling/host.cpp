...
int main(int argc, char** argv)
{
	...
	cl_command_queue commands0 = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err);
...
#define NUM_KERNELS 3
	...
	{
		err = clEnqueueNDRangeKernel(commands0, denoise_kernels[0], dim0, NULL, global0, local0, 0, NULL, &enqueue_event);
		...
		cl_ulong ctr_start, ctr_end;
		size_t return_bytes1 = 0, return_bytes2 = 0;

		//error = clGetEventProfilingInfo(enqueue_event, CL_PROFILING_COMMAND_SUBMIT, sizeof(cl_ulong), &ctr_start, NULL);
		err = clGetEventProfilingInfo(enqueue_event, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &ctr_start, &return_bytes2);
		err |= clGetEventProfilingInfo(enqueue_event, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &ctr_end, &return_bytes2);

		if (err != CL_SUCCESS) {
			printf("Error in clGetEventProfilingInfo: Error number %d \n", err);
		}
		//kernel execution time obained is in nano seconds
		kernel_execution_time[rpts] = (ctr_end - ctr_start);
	...
	printf("OpenCL (OCL) event-based command profiling .\n");
	printf("Average execution time %f ms.\n", avg_execution_time / 1000000);
	...
	for (int rpts = 0; rpts < MAX_IERATIONS; rpts++)
	{
		err = clEnqueueNDRangeKernel(commands0, denoise_kernels[2], dim0, NULL, global0, local0, 0, NULL, &enqueue_event);
